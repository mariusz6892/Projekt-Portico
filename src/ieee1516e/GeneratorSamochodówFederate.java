 ...
 */
public class GeneratorSamochodówFederate
{
	//----------------------------------------------------------
	//                    STATIC VARIABLES
	//----------------------------------------------------------
	...
	private GeneratorSamochodówAmbassador fedamb;  // created when we connect
	...
	//ZMIENNE DODANE
	private Samochód[] samochody = new Samochód[ITERATIONS];
	...
	{
		System.out.println( "GeneratorSamochodów   : " + message );
	}
	...
	public void runFederate( String federateName ) throws Exception
	{
		...
		// connect
		log( "Connecting..." );
		fedamb = new GeneratorSamochodówAmbassador( this );
		...
		for( int i = 0; i < ITERATIONS; i++ )
		{
			...
			//z prawdopodobnieñstwem 9/10 generujê samochody (w ka¿dej sekundzie czasu symulacyjnego mo¿e zostaæ wygenerowany samochód)
			if (new Random().nextInt(100) < 90) {
				samochody[nr_samochodu] = new Samochód(nr_samochodu,true);
				updateAttributeValues( objectHandle, nr_samochodu );
				System.out.println("Wygenerowa³em samochód o numerze " + nr_samochodu + " znajduj¹cy siê na drodzê numer " + samochody[nr_samochodu].getAktualna_pozycja() + " jad¹cy w stronê drogi numer " + samochody[nr_samochodu].getKierunek_jazdy());
				if (samochody[nr_samochodu].isUprzywilejowany()) System.out.println("Jest pojazdem uprzywilejowanym");
				nr_samochodu++;
			}
	...
	private void publishAndSubscribe() throws RTIexception
	{
		...
		rtiamb.publishObjectClassAttributes( samochodHandle, attributes );
	}
	...
	{
		return rtiamb.registerObjectInstance( samochodHandle );
	}
	 ...
	 * @param nr_samochodu 
	 */
	private void updateAttributeValues( ObjectInstanceHandle objectHandle, int nr_samochodu ) throws RTIexception
	{
		///////////////////////////////////////////////
		...
		HLAinteger32BE nr_samochoduValue = encoderFactory.createHLAinteger32BE( nr_samochodu );
		attributes.put( nr_samochoduHandle, nr_samochoduValue.toByteArray() );
		
		hla.rti1516e.encoding.HLAboolean uprzywilejowanyValue = encoderFactory.createHLAboolean( samochody[nr_samochodu].isUprzywilejowany());
		attributes.put( uprzywilejowanyHandle, uprzywilejowanyValue.toByteArray() );
		
		HLAinteger32BE kierunek_jazdyValue = encoderFactory.createHLAinteger32BE( samochody[nr_samochodu].getKierunek_jazdy() );
		attributes.put( kierunek_jazdyHandle, kierunek_jazdyValue.toByteArray() );
		
		HLAinteger32BE aktualna_pozycjaValue = encoderFactory.createHLAinteger32BE( samochody[nr_samochodu].getAktualna_pozycja() );
		attributes.put( aktualna_pozycjaHandle, aktualna_pozycjaValue.toByteArray() );
	...
	public static void main( String[] args )
	{
		...
		String federateName = "GeneratorSamochodów";
		if( args.length != 0 )
		{
			federateName = args[0];
		}
		...
		try
		{
			...
			new GeneratorSamochodówFederate().runFederate( federateName );
		}
