 ...
 */
public class SygnalizacjaFederate
{
	//----------------------------------------------------------
	//                    STATIC VARIABLES
	//----------------------------------------------------------
	...
	private SygnalizacjaAmbassador fedamb;  // created when we connect
	...
	private Sygnalizator[] sygnalizatory = new Sygnalizator[2];
	...
	{
		System.out.println( "Sygnalizacja   : " + message );
	}
	...
	public void runFederate( String federateName ) throws Exception
	{
		...
		// connect
		log( "Connecting..." );
		fedamb = new SygnalizacjaAmbassador( this );
		...
		// send an interaction.
		//utworzenie sygnalizatorów oraz zainicjowanie ich œwiate³
		
		//Sygnalizatory s¹ przyporz¹dkowane parami, dla drogi o numerze 0 oraz 2 przyporz¹dkowana jest grupa sygnalizatorów o numerze 0, natomiast dla dróg 1,3 grupa sygnalizatorów o numerze 1.
		//Jest to spowodowane tym, i¿ w przypadku zapalenia koloru zielonego dla drogi 0, droga numer 2 automatycznie zapala to samo œwiat³o, dla uproszczenia zrobi³em wiêc 2 grupy sygnalizatorów, ni¿ 4 osobne.
		
		sygnalizatory[0] = new Sygnalizator(0);
		sygnalizatory[0].setZielone(true); //zielone
		
		sygnalizatory[1] = new Sygnalizator(1);
		sygnalizatory[1].setZielone(false); //zielone
		
		
		for( int i = 0; i < ITERATIONS; i++ )
		{
			// 9.1 update the attribute values of the instance //
			//zmiana œwiate³ wystêpuje w odstêpie 5 jednostki czasu symulacyjnego
			if (i%5 == 0) {
				if (sygnalizatory[0].getZielone()) { //jezeli sygnalizator byl zielony zmieniamy na kolor przeciwny
					sygnalizatory[0].setZielone(false);
					System.out.println("Sygnalizator numer 0 zmienia œwiat³o na czerwone");
					sygnalizatory[1].setZielone(true);
					System.out.println("Sygnalizator numer 1 zmienia œwiat³o na zielone");
				}
				else {
					sygnalizatory[0].setZielone(true);
					System.out.println("Sygnalizator numer 0 zmienia œwiat³o na zielone");
					sygnalizatory[1].setZielone(false);
					System.out.println("Sygnalizator numer 1 zmienia œwiat³o na czerwone");
				}
				updateAttributeValues(objectHandle,0,sygnalizatory[0].getZielone());
				updateAttributeValues(objectHandle,1,sygnalizatory[1].getZielone());
			}
	...
	private void publishAndSubscribe() throws RTIexception
	{
		...
		attributes.add( nr_sygnalizatoraHandle );
		attributes.add( swiatloHandle );
		
		// do the actual publication
		rtiamb.publishObjectClassAttributes( sygnalizatorHandle, attributes );
	}
	...
	{
		return rtiamb.registerObjectInstance( sygnalizatorHandle );
	}
	 ...
	 */
	private void updateAttributeValues( ObjectInstanceHandle objectHandle, int nrsygnalizatora, boolean zielone ) throws RTIexception
	{
		///////////////////////////////////////////////
		...
		HLAinteger32BE nrsygnalizatoraValue = encoderFactory.createHLAinteger32BE( nrsygnalizatora );
		attributes.put( nr_sygnalizatoraHandle, nrsygnalizatoraValue.toByteArray() );
		
		hla.rti1516e.encoding.HLAboolean swiatloValue = encoderFactory.createHLAboolean (zielone);
		attributes.put( swiatloHandle, swiatloValue.toByteArray() );
	...
	public static void main( String[] args )
	{
		...
		String federateName = "Sygnalizacja";
		if( args.length != 0 )
		{
			federateName = args[0];
		}
		...
		try
		{
			...
			new SygnalizacjaFederate().runFederate( federateName );
		}
