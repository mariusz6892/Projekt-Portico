 ...
 */
public class Skrzy¿owanieFederate
{
	//----------------------------------------------------------
	//                    STATIC VARIABLES
	//----------------------------------------------------------
	...
	private Skrzy¿owanieAmbassador fedamb;  // created when we connect
	private HLAfloat64TimeFactory timeFactory; // set when we join
	protected EncoderFactory encoderFactory;     // set when we join

	// caches of handle types - set once we join a federation
	
	//subskrybowane z samochodu
	protected ObjectClassHandle samochodHandle;
	protected AttributeHandle nr_samochoduHandle;
	protected AttributeHandle uprzywilejowanyHandle;
	protected AttributeHandle kierunek_jazdyHandle;
	protected AttributeHandle aktualna_pozycjaHandle;
	
	public Samochód[] samochody = new Samochód[ITERATIONS];
	
	//subskrybowane z sygnalizacji
	protected ObjectClassHandle sygnalizatorHandle;
	protected AttributeHandle nr_sygnalizatoraHandle;
	protected AttributeHandle swiatloHandle;

	public Sygnalizator[] sygnalizatory = new Sygnalizator[2];
	
	public boolean Sygnalizator_semafor = false;
	
	//KOLEJKI
	public Droga[] drogi = new Droga[4];
	...
	{
		System.out.println( "Skrzy¿owanie   : " + message );
	}
	...
	public void runFederate( String federateName ) throws Exception
	{
		...
		// connect
		log( "Connecting..." );
		fedamb = new Skrzy¿owanieAmbassador( this );
		...
		*/
		...
		{
			if (Sygnalizator_semafor) {
				if ((((drogi[0].kieruneksamochodu() == 2) || (drogi[0].kieruneksamochodu() == 3)) && (sygnalizatory[0].getZielone())) || (drogi[0].kieruneksamochodu() == 5)) drogi[0].jedz(); //z 0 jedzie prosto lub w prawo
				else if ((drogi[0].kieruneksamochodu() == 1) && ((drogi[2].ilewkolejce() == 0) || (drogi[2].kieruneksamochodu() == 3)) && (sygnalizatory[0].getZielone())) drogi[0].jedz(); //z 0 w lewo
				
				if ((((drogi[1].kieruneksamochodu() == 3) || (drogi[1].kieruneksamochodu() == 0)) && (sygnalizatory[1].getZielone())) || (drogi[1].kieruneksamochodu() == 5)) drogi[1].jedz(); //z 1 jedzie prosto lub w prawo
				else if ((drogi[1].kieruneksamochodu() == 2) && ((drogi[3].ilewkolejce() == 0) || (drogi[3].kieruneksamochodu() == 0)) && (sygnalizatory[1].getZielone())) drogi[1].jedz();// z 1 w lewo
				
				if ((((drogi[2].kieruneksamochodu() == 0) || (drogi[2].kieruneksamochodu() == 1)) && (sygnalizatory[0].getZielone())) || (drogi[2].kieruneksamochodu() == 5)) drogi[2].jedz(); //z 2 jedzie prosto lub w prawo
				else if ((drogi[2].kieruneksamochodu() == 3) && ((drogi[0].ilewkolejce() == 0) || (drogi[0]).kieruneksamochodu() == 1) && (sygnalizatory[0].getZielone() )) drogi[2].jedz();// z 2 w lewo
				
				if ((((drogi[3].kieruneksamochodu() == 1) || (drogi[3].kieruneksamochodu() == 2)) && (sygnalizatory[1].getZielone())) || (drogi[3].kieruneksamochodu() == 5)) drogi[3].jedz(); //z 3 jedzie prosto lub w prawo
				else if ((drogi[3].kieruneksamochodu() == 0) && ((drogi[1].ilewkolejce() == 0) || (drogi[1].kieruneksamochodu() == 2)) && (sygnalizatory[1].getZielone())) drogi[3].jedz();// z 3 w lewo
				System.out.println("Wielkosc kolejki drogi nr 0 " + drogi[0].ilewkolejce());
				System.out.println("Wielkosc kolejki drogi nr 1 " + drogi[1].ilewkolejce());
				System.out.println("Wielkosc kolejki drogi nr 2 " + drogi[2].ilewkolejce());
				System.out.println("Wielkosc kolejki drogi nr 3 " + drogi[3].ilewkolejce());	
			}
	...
	private void publishAndSubscribe() throws RTIexception
	{
		...
		this.sygnalizatorHandle = rtiamb.getObjectClassHandle( "HLAobjectRoot.Sygnalizator" );
		this.nr_sygnalizatoraHandle = rtiamb.getAttributeHandle( sygnalizatorHandle, "Nr_sygnalizatora" );
		this.swiatloHandle = rtiamb.getAttributeHandle( sygnalizatorHandle, "Swiatlo" );
		// package the information into a handle set
		AttributeHandleSet attributessygn = rtiamb.getAttributeHandleSetFactory().create();
		attributessygn.add( nr_sygnalizatoraHandle );
		attributessygn.add( swiatloHandle );

		this.samochodHandle = rtiamb.getObjectClassHandle( "HLAobjectRoot.Samochod" );
		this.nr_samochoduHandle = rtiamb.getAttributeHandle( samochodHandle, "Nr_samochodu" );
		this.uprzywilejowanyHandle = rtiamb.getAttributeHandle( samochodHandle, "Uprzywilejowany" );
		this.kierunek_jazdyHandle = rtiamb.getAttributeHandle( samochodHandle, "Kierunek_jazdy" );
		this.aktualna_pozycjaHandle = rtiamb.getAttributeHandle( samochodHandle, "Aktualna_pozycja" );
		// package the information into a handle set
		AttributeHandleSet attributes = rtiamb.getAttributeHandleSetFactory().create();
		attributes.add( nr_samochoduHandle );
		attributes.add( uprzywilejowanyHandle );
		attributes.add( kierunek_jazdyHandle );
		attributes.add( aktualna_pozycjaHandle );
		...
		rtiamb.subscribeObjectClassAttributes( sygnalizatorHandle, attributessygn );
		rtiamb.subscribeObjectClassAttributes( samochodHandle, attributes );
	}
	...
	public static void main( String[] args )
	{
		...
		String federateName = "Skrzy¿owanie";
		if( args.length != 0 )
		{
			federateName = args[0];
		}
		...
		try
		{
			...
			new Skrzy¿owanieFederate().runFederate( federateName );
		}
